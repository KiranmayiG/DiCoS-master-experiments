...
class Generator(nn.Module):
    def __init__(self, args, n_op,  n_domain,  update_id, ans_vocab, slot_mm, turn = 2, turn_id = 30006):
        ...
        # self.albert = BertModel.from_pretrained(args.model_name_or_path + "pytorch_model.bin",  config = bert_config)
        ...
        self.albert = torch.nn.DataParallel(self.albert, device_ids = [0])
    ...
    def forward(self,  input_ids,  token_type_ids,
        ...
        state_output = state_output.masked_select(torch.eye(self.n_slot).to(device).unsqueeze(0).unsqueeze(-1)==1).reshape(sequence_output.shape[0], self.n_slot, 1, -1)
    ...
    def history_gate(self, text):
        ...
        mask = (indice_mat - torch.triu(indice_mat, diagonal=1)).to(device)
    ...
    def select_history(self, fuse_score, history, topk, slot_mm, sample_mm):
        ...
        indices_mat = torch.ones((fuse_score.shape[0], fuse_score.shape[0])).to(device)
        ...
        max_related_indices = torch.cat((max_related_indices,torch.linspace(0, turn - 1, turn).unsqueeze(-1).unsqueeze(-1).repeat(1, self.n_slot, 1).long().to(device)), dim=-1)
        ...
        history_mask = sample_mm.bmm(torch.cat(history_masks, dim = 0).reshape(max_related_indices.shape[:2] + tuple([seqlen])).to(device).float())
        history = sample_mm.bmm(torch.cat(histories, dim=0).reshape(max_related_indices.shape[:2] + tuple([seqlen])).to(device).float())
        history_slot_mask = torch.cat(history_slot_masks, dim=0).reshape(max_related_indices.shape[:2] + tuple([seqlen])).to(device).float()
        history_turn_mask = torch.cat(history_turn_masks, dim=0).reshape(max_related_indices.shape[:2] + tuple([seqlen])).to(device).float()
        token_ids = sample_mm.bmm(torch.cat(token_ids, dim=0).reshape(max_related_indices.shape[:2] + tuple([seqlen])).to(device).float())
        state_position = torch.cat(state_positions, dim=0).reshape(max_related_indices.shape[:2] + tuple([-1])).to(device).float()
